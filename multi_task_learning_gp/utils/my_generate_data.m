function [x, Y, xtrain, ytrain, ind_kf_train, ind_kx_train, nx] = ...
                                    my_generate_data(covfunc_x,M,min_range,max_range,n_points,rho,sigma_2n)
...
% - M         : Number of tasks
% - rho       : Correlation between problems
% - sigma_2n  : Noise variance
% - min_range : min grid value
% - max_range : max grid value
%
% OUTPUT:    
% - x             : all inputs
% - Y             : all target values
...
%                each observation y corresponds to for training
% - ind_kx_train  : Vector containing the indexes of the data-points in x
%                which each observation y corresponds to for training
...
%% Input 2D Point  
range = linspace(min_range,max_range, n_points)'; % Alternative
